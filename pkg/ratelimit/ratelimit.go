...
func NewBucketWithRateAndClock(rate float64, capacity int64, clock Clock) *Bucket {
	...
	log.Panic().Msgf("cannot find suitable quantum for %s", strconv.FormatFloat(rate, 'g', -1, 64))
	return nil
}
...
func NewBucketWithQuantumAndClock(fillInterval time.Duration, capacity, quantum int64, clock Clock) *Bucket {
	...
	if fillInterval <= 0 {
		log.Panic().Msg("token bucket fill interval is not > 0")
	}
	if capacity <= 0 {
		log.Panic().Msg("token bucket capacity is not > 0")
	}
	if quantum <= 0 {
		log.Panic().Msg("token bucket quantum is not > 0")
	}
	return &Bucket{
		clock:           clock,
		startTime:       clock.Now(),
